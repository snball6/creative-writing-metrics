...
ggplot(y_24_25_date_grouped, aes(x = sum_hours, fill=highlight)) +
  ...
  scale_fill_manual(values = c("Highlighted" = highight_c, "Normal" = base_color)) +
  theme_minimal() +
  theme(
...
cat_hist %>%
  ...
  labs(
    title = "Normalized distribution of writing session length for 2024",
    x = "Hours written",
    y = "Density"
  ) +
  theme_minimal() +
...
#TODO REVISIT - probably not collapsing correctly...
...
ggplot(model_year_monthly, aes(x = month, y = total_hours, fill = hour_set)) +
  ...
  scale_fill_manual(
    values = c("cat_adjust_reduction" = reduct_c, "cat_adjust_hours" = base_color),
...
model_year_daily <- model_year %>%
  ...
  summarise(
    ...
    total_cat_adjust_hours = sum(cat_adjust_hours), 
    total_cat_adjust_reduction = sum(cat_adjust_reduction),
    .groups = 'drop') %>%
  mutate(
...
ggplot(model_year_daily, aes(x = date, y = total_original_hours)) +
  geom_point(color = base_color) +
  labs(title = "Daily writing session length",
       x = "",
       y = "Hours") +
  theme_minimal()
...
As you can see in the above, my writing hours are highly variable from day-to-day. I knew that the spike up to 8 hours in June 2024occurred during a vacation week focused almost exclusively on creative writing, but I did not know how many other high or low writing days could be tied to vacation time or sickdays. To determine this I enriched my dataset with more detailed information about my work schedule. For more on how I sourced and cleaned my schedule data from my old Menlo timesheets, see [**Python & Data Cleaning**](#python-and-data-cleaning).

This data enrichment helped me confirm that paid leave days in general--both sick day and vacation--could include high outliers or near-zero days. With these days excluded I could calculate my median weekend and weekday writing speed.
...
merged <- merged %>% 
  mutate(schedule_category = case_when(
...
pto_median = median(merged$total_original_hours[merged$schedule_category == "paid leave"], na.rm = TRUE)
weekend_median = median(merged$total_original_hours[merged$schedule_category == "workday"], na.rm = TRUE)
weekday_median = median(merged$total_original_hours[merged$schedule_category == "weekend"], na.rm = TRUE)
...
merged %>%
  ...
  facet_wrap(~ schedule_category, labeller = facet_labeller) +
  labs(
    title = "Normalized distribution of writing session",
    x = "Hours written",
    y = "Density"
  ) +
  theme_minimal()
...
I calculated the medians both before and after the 3rd+ Draft category adjustment and found it resulted in a shift of the weekday median down by 0.25 to 1. I chose to use the higher pre-adjustment median for my substitution.

I tried both a Friday replacement and a Sunday replacement strategy and ultimately chose Sunday. Because my weekend habit is usually to spend the first day on chores and life admin tasks, and then do more hobby and creative tasks the next day--or two in the case of 2024, so replacing Sunday would be the more reflective of my 2026 schedule.

```{r schedule corrected, fig.height=3, fig.width=12}

model_year_daily$sched_adjust_hours = ifelse(model_year_daily$day_of_week == "Sunday", 
                                             1, 
                                             model_year_daily$total_cat_adjust_hours)

model_year_daily$sched_adjust_reduction = model_year_daily$total_cat_adjust_hours - model_year_daily$sched_adjust_hours

model_year_long <- model_year_daily %>%
  pivot_longer(
...
```{r model year weekly, fig.height=3, fig.width=12}

model_year_weekly <- model_year_daily %>%
  group_by(week_ending) %>%
  summarise(
    total_adj_hours = sum(sched_adjust_hours), 
    total_orig_hours = sum(total_original_hours), 
    .groups = "drop")

ggplot(model_year_weekly, aes(x = week_ending, y = total_orig_hours)) +
  geom_col(color = "#ffffff",
           fill = base_color
           ) +
  labs(title = "2024 Original Weekly Total Hours",
       x = "",
       y = "") +
  theme_minimal() 

ggplot(model_year_weekly, aes(x = week_ending, y = total_adj_hours)) +
  geom_col(color = "#ffffff",
           fill = base_color
           ) +
  labs(title = "Adjusted Model Year Weekly Total Hours",
       x = "",
       y = "") +
  theme_minimal() 

baseline_total = sum(model_year_weekly$total_orig_hours, na.rm=TRUE)
baseline_mean = mean(model_year_weekly$total_orig_hours, na.rm=TRUE)
baseline_median = median(model_year_weekly$total_orig_hours, na.rm=TRUE)


adjusted_total = sum(model_year_weekly$total_adj_hours, na.rm=TRUE)
adjusted_mean = mean(model_year_weekly$total_adj_hours, na.rm=TRUE)
adjusted_median = median(model_year_weekly$total_adj_hours, na.rm=TRUE)
```

With both sets of adjustments in place, my model year consists of

|              | 2024   | Model  | Reduction |
|:-------------|:-------|:-------|:----------|
| Weekly mean  | 9.5    | 8      | -1.5      |
| Yearly total | 554.25 | 432.25 | -122      |

## What ifs

To identify experiments I could run to hit my model year numbers, I ran what if scenarios against my lower 2025 data set to see which ones might have allowed me to hit my model year output.
